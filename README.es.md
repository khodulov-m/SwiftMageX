...
que se publicó en la v0.3.0 está en `RELEASE_NOTES.md`.
...
| `--version` | Imprime `0.3.0` y termina. |
...
Envía una imagen de origen (más cualquier referencia adicional y una máscara
opcional) a un modelo Gemini junto con el prompt de texto. Cada imagen viaja
como parte `inlineData` dentro de la misma llamada `:generateContent` que usa
`generate` — sin nuevo endpoint, sin nueva dependencia.
...
| Opción | Predeterminado | Notas |
|---|---|---|
| `<input>` | — | Obligatorio. Imagen de origen primaria (PNG o JPEG). |
| `<prompt>` | — | Obligatorio. Instrucción de texto que describe la edición. |
| `--reference <ruta>` | — | Repetible. Imagen de referencia adicional (PNG o JPEG). Cada `--reference` añade otra parte inline que el prompt puede combinar — p. ej., "lleva el sujeto de la imagen 1 a la escena 2". |
| `--mask <ruta>` | — | Máscara opcional en escala de grises o binaria (PNG o JPEG). El blanco marca la región a editar sobre el `<input>` primario; el negro la preserva. |
| `-o`, `--output <ruta>` | `./` | Archivo o directorio. Si es directorio, los archivos se nombran `swiftmagex_{timestamp}_{index}.png`. |
| `-n`, `--count <1–4>` | `1` | Cantidad de variantes. Cada una es una petición separada. |
| `--seed <uint64>` | — | Se registra en metadatos incluso si el proveedor lo ignora. |
...
# Composición con dos imágenes de referencia
...
Ejemplos — tres pares antes / después (fuentes y ediciones en
[`examples/`](examples)):

| Fuente | Editada | Prompt de edición |
|---|---|---|
| <img src="examples/apple.png" alt="Manzana roja sobre fondo blanco" width="200"> | <img src="examples/apple-edited.png" alt="Manzana coloreada de verde brillante" width="200"> | `"Change the apple's color from red to bright green, keep everything else identical"` |
| <img src="examples/mountain.png" alt="Lago de montaña al amanecer" width="200"> | <img src="examples/mountain-edited.png" alt="La misma escena con un globo aerostático sobre las cumbres" width="200"> | `"Add a single colorful hot-air balloon floating in the sky above the mountains"` |
| <img src="examples/cabin.png" alt="Cabaña de madera en un bosque veraniego" width="200"> | <img src="examples/cabin-edited.png" alt="La misma cabaña bajo la nieve" width="200"> | `"Transform the scene from a sunny summer day to a snowy winter day"` |

Composición multi-imagen — combina un sujeto con una escena vía `--reference`:

| Entrada (sujeto) | Referencia (escena) | Resultado |
|---|---|---|
| <img src="examples/canoe.png" alt="Canoa de madera sobre fondo blanco" width="200"> | <img src="examples/mountain.png" alt="Lago de montaña al amanecer" width="200"> | <img src="examples/canoe-on-mountain-lake.png" alt="Canoa flotando en el lago de montaña" width="200"> |
...
Pasa `--cache-dir <ruta>` a `generate` o `edit` para cortocircuitar la
llamada de red cuando una solicitud idéntica ya se ha servido. La caché es
content-addressed: la clave es un SHA-256 sobre modelo, prompt, tamaño,
count, seed, más el SHA-256 de cada imagen de referencia y de los bytes
de la máscara. En un hit los bytes se reproducen desde disco y aun así se
escribe un archivo de salida con metadatos `tEXt`/EXIF actualizados
(timestamp, versión de la herramienta), de modo que los pipelines aguas
abajo se comportan igual.

```sh
# La primera llamada va a la API; la segunda reproduce desde /tmp/sx-cache.
...
# → "cached": true en la salida JSON por cada imagen reproducida
```

Notas:

- **Opt-in por diseño.** Gemini no respeta `--seed`, así que entradas
  idénticas están *pensadas* para variar entre llamadas — un cache hit
  convierte silenciosamente esa variabilidad intencional en "los mismos
  bytes cada vez". Pasa `--cache-dir` solo si eso es lo que quieres.
- **Best-effort.** Los fallos de I/O de caché (directorio no escribible,
  entrada corrupta) degradan a una llamada de red normal en vez de
  abortar el comando.
- **Sin eviction.** La caché crece hasta que hagas `rm -rf` del directorio.
- La superficie de caché aplica solo a `generate` / `edit` (los comandos
  de raster local no llaman a ningún proveedor). El servidor MCP no expone
  caché en 0.1; la opción CLI es la única vía hoy.
...
### `swiftmagex icon` — paquetes `.icon` de Icon Composer

Ensambla un paquete `.icon` de
[Icon Composer](https://developer.apple.com/icon-composer/) — el formato
de íconos de app en capas con Liquid Glass para iOS 26+ / macOS 26+ — a
partir de imágenes de capa preparadas. Totalmente local, sin clave. Las
capas se listan de abajo hacia arriba y se apilan sobre el lienzo de
1024 pt de Icon Composer; el comando escribe `icon.json` más `Assets/` y
reporta la ruta absoluta del paquete. Prepara las capas antes con
...
Las opciones por capa se añaden a la ruta, separadas por comas: `name=…`,
`glass=true|false` (Liquid Glass, true por defecto), `scale=N`
(multiplicador del tamaño natural de la capa, 1 píxel fuente = 1 punto),
`dx=N` / `dy=N` (desplazamiento en puntos desde la posición centrada,
positivo = derecha/abajo), `fill=#RRGGBB[AA]` (tinte sólido), `group=N`
(basado en 1; las capas de un grupo deben ser contiguas, máx. 4 por
grupo).

| Opción | Por defecto | Notas |
|---|---|---|
| `<layer>...` | — | De abajo hacia arriba. Se recomienda PNG con transparencia; otros formatos se recodifican a PNG. |
| `-o`, `--output <path>` | `AppIcon.icon` | Se añade `.icon` si falta. |
| `--fill <solid:#HEX\|auto:#HEX>` | `solid:#FFFFFF` | Fondo del ícono; `auto:` es el degradado del sistema sembrado con un color. |
| `--overwrite` | off | Reemplaza atómicamente un paquete existente. |
| `--flat-preview` | off | Escribe además un compuesto plano PNG de 1024×1024 (`<name>-flat.png`) — sin Liquid Glass, sin máscara squircle — para READMEs y consumidores sin Xcode. |
| `--flat-preview-output <path>` | junto al paquete | |
| `--validate` | off | Verifica el paquete compilándolo con `actool` de Xcode (requiere Xcode 26). Exit 4 si falta actool, exit 2 si el paquete no compila. |

```sh
# Fondo + marca de vidrio, relleno degradado, con vista previa y chequeo de compilación
swiftmagex icon bg.png mark.png,scale=0.8,dy=-20 \
  --fill auto:#7B1FA2 -o AppIcon.icon --flat-preview --validate

# Insignia anclada abajo a la derecha, sin vidrio, tinte blanco, grupo propio
swiftmagex icon art.png badge.png,glass=false,fill=#FFFFFF,dx=222,dy=223,group=2
```

Arrastra el `AppIcon.icon` resultante a un proyecto de Xcode 26 (o ábrelo
en Icon Composer) — Xcode renderiza el efecto Liquid Glass y genera las
variantes planas para versiones anteriores del SO.
...
`swiftmagex-mcp` expone diez herramientas — `generate_image`, `edit_image`,
`resize_image`, `overlay_text`, `composite_images`, `appstore_screenshots`,
`list_frames`, `remove_background`, `smart_crop` y `compose_icon` — sobre
stdio. Los argumentos espejan
...
acepta el argumento `frame` de `appstore_screenshots`. `compose_icon`
espeja `swiftmagex icon` (capas como array de objetos) y devuelve la
vista previa plana como contenido de imagen cuando se pasa
`flat_preview` — el flujo natural del agente es `generate_image` →
...
`.icon` de Icon Composer). Cualquier cosa
