...
`gemini-2.5-flash-image` is the stable all-rounder; the newest
`gemini-3.1-flash-image-preview` produces noticeably more detailed results
and picks wider, non-square framings on its own; the Imagen family offers
explicit aspect-ratio control (`--size` maps to `aspectRatio`), with
`imagen-4.0-fast-generate-001` tuned for speed and
`imagen-4.0-ultra-generate-001` for maximum fidelity (one image per call).
For example, `gemini-3.1-flash-image-preview` produced this 1408×768 frame:
...
<img src="docs/images/example-generate-fox-gemini31.png" alt="Detailed fox in an autumn forest, generated by gemini-3.1-flash-image-preview" width="480" />
