...
enum ToolHandlers {
    ...
    static func generate(
        ...
        do {
            ...
            // Spec §7: echo the generated bytes back as MCP image content so
            ...
            content.append(contentsOf: imageContents(for: written))
    ...
    static func edit(
        ...
        do {
            ...
            // Spec §7: echo the edited bytes back as MCP image content so the
    ...
    /// the produced bytes as MCP image content so the calling model can see
    /// the output, not just a path it may be unable to read. Used by
    /// `composite_images`, `appstore_screenshots`, `remove_background`, and
    /// `smart_crop`. `resize_image` / `overlay_text` deliberately don't call
    /// this — the agent supplied their input, so only the path is returned
    /// (spec §7).
    private static func result(for images: [WrittenImage]) -> CallTool.Result {
        var content: [Tool.Content] = [
            .text(text: summaryText(for: images), annotations: nil, _meta: nil)
        ]
        content.append(contentsOf: imageContents(for: images))
        return CallTool.Result(content: content, isError: false)
    }

    /// Maps each written image to an MCP image-content block. Best effort: a
    /// file that can't be re-read is silently skipped — its path is still in
    /// the text summary, so the agent isn't left without a reference.
    private static func imageContents(for images: [WrittenImage]) -> [Tool.Content] {
        images.compactMap { image in
            guard let data = try? Data(contentsOf: image.path) else { return nil }
            return .image(
