...
- **`swiftmagex`** (executable) — CLI frontend. Depends on `SwiftMageXKit` + `swift-argument-parser`. Subcommands: `generate`, `resize`, `text`, `composite`, `appstore`, `remove-bg`, `crop` (the last four post-0.1). Global flags: `--json`, `--verbose`.
- **`swiftmagex-mcp`** (executable) — MCP server frontend over stdio. Depends on `SwiftMageXKit` + the MCP Swift SDK. Exposes eight tools (`generate_image`, `resize_image`, `overlay_text`, `composite_images`, `appstore_screenshots`, `list_frames`, `remove_background`, `smart_crop`) that mirror the CLI commands; `list_frames` is the discovery counterpart to `appstore_screenshots`'s bundled-bezel ids.
