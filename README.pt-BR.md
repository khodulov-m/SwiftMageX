...
Exemplo — a chamada abaixo gerou a imagem a seguir (PNG 1408×768 com
...
**Escolha o modelo conforme suas exigências de qualidade.** Os modelos são
intercambiáveis a cada chamada via `--model` — nada mais muda no comando. O
padrão `gemini-3.1-flash-image` é o pau-para-toda-obra: resultados
detalhados, e escolhe por conta própria enquadramentos mais largos, não
quadrados. `gemini-3.1-flash-lite-image` é a opção mais barata e rápida para
rascunhos, e `gemini-3-pro-image` busca fidelidade máxima. A família Imagen
ficava aqui; o Google a desativou em 17 de agosto de 2026 e qualquer ID
`imagen-*` agora retorna 404, então escolha um dos três acima. Por exemplo,
`gemini-3-pro-image` produziu este quadro de 1408×768:
...
<img src="docs/images/example-generate-fox-gemini3-pro.png" alt="Raposa detalhada em uma floresta de outono, gerada por gemini-3-pro-image" width="480" />

### `swiftmagex edit` — imagem-para-imagem / multi-imagem / inpainting via Gemini

Envia uma imagem de origem (mais quaisquer referências adicionais e uma
máscara opcional) para um modelo Gemini junto com o prompt de texto. Cada
imagem viaja como parte `inlineData` na mesma chamada `:generateContent`
que `generate` usa — sem novo endpoint, sem nova dependência.
...
| Opção | Padrão | Notas |
|---|---|---|
| `<input>` | — | Obrigatório. Imagem de origem primária (PNG ou JPEG). |
| `<prompt>` | — | Obrigatório. Instrução de texto descrevendo a edição. |
| `--reference <caminho>` | — | Repetível. Imagem de referência adicional (PNG ou JPEG). Cada `--reference` adiciona outra parte inline que o prompt pode compor — ex.: "leve o sujeito da imagem 1 para a cena da imagem 2". |
| `--mask <caminho>` | — | Máscara opcional em tons de cinza ou binária (PNG ou JPEG). Branco marca a região a editar na imagem primária; preto preserva o original. |
...
# Composição entre duas imagens de referência
...
Passe `--cache-dir <caminho>` para `generate` ou `edit` para
curto-circuitar a chamada de rede quando uma requisição idêntica já tiver
sido servida. O cache é content-addressed: a chave é um SHA-256 sobre
model, prompt, size, count, seed, mais o SHA-256 de cada imagem de
referência e dos bytes da mask. Em um hit, os bytes são reproduzidos do
disco e mesmo assim um arquivo de saída é gravado com metadados
`tEXt`/EXIF frescos (timestamp, versão da ferramenta), para que pipelines
à jusante se comportem da mesma forma.

```sh
# Primeira chamada vai à API; a segunda replica de /tmp/sx-cache.
...
# → "cached": true na saída JSON para cada imagem replicada
```

Ressalvas:

- **Opt-in por design.** O Gemini não honra `--seed`, então entradas
  idênticas são *para* variar entre chamadas — um cache hit converte
  silenciosamente esse não-determinismo intencional em "os mesmos bytes
  toda vez". Passe `--cache-dir` só se é isso que você quer.
- **Best-effort.** Falhas de I/O do cache (diretório sem permissão de
  escrita, entrada corrompida) degradam para uma chamada de rede normal
  em vez de abortar o comando.
- **Sem eviction.** O cache cresce até você fazer `rm -rf` do diretório.
- O cache só vale para `generate` / `edit` (os comandos de raster local
  não chamam provedor). O servidor MCP não expõe cache em 0.1; o flag
  CLI é a única porta de entrada hoje.
...
### `swiftmagex icon` — pacotes `.icon` do Icon Composer

Monta um pacote `.icon` do
[Icon Composer](https://developer.apple.com/icon-composer/) — o formato
de ícones de app em camadas com Liquid Glass para iOS 26+ / macOS 26+ —
a partir de imagens de camada preparadas. Totalmente local, sem chave.
As camadas são listadas de baixo para cima e empilhadas na tela de
1024 pt do Icon Composer; o comando grava `icon.json` mais `Assets/` e
informa o caminho absoluto do pacote. Prepare as camadas antes com
...
As opções por camada são anexadas ao caminho, separadas por vírgula:
`name=…`, `glass=true|false` (Liquid Glass, padrão true), `scale=N`
(multiplicador do tamanho natural da camada, 1 pixel de origem =
1 ponto), `dx=N` / `dy=N` (deslocamento em pontos a partir do
posicionamento centralizado, positivo = direita/baixo),
`fill=#RRGGBB[AA]` (tinta sólida), `group=N` (a partir de 1; camadas de
um grupo devem ser contíguas, máx. 4 por grupo).

| Opção | Padrão | Notas |
|---|---|---|
| `<layer>...` | — | De baixo para cima. PNG com transparência recomendado; outros formatos são recodificados para PNG. |
| `-o`, `--output <path>` | `AppIcon.icon` | `.icon` é acrescentado quando ausente. |
| `--fill <solid:#HEX\|auto:#HEX>` | `solid:#FFFFFF` | Fundo do ícone; `auto:` é o gradiente do sistema derivado de uma cor. |
| `--overwrite` | desligado | Substitui atomicamente um pacote existente. |
| `--flat-preview` | desligado | Grava também um composto plano PNG 1024×1024 (`<name>-flat.png`) — sem Liquid Glass, sem máscara squircle — para READMEs e consumidores sem Xcode. |
| `--flat-preview-output <path>` | ao lado do pacote | |
| `--validate` | desligado | Verifica o pacote compilando com o `actool` do Xcode (requer Xcode 26). Exit 4 se o actool faltar, exit 2 se o pacote não compilar. |

```sh
# Fundo + marca de vidro, preenchimento em gradiente, com prévia e checagem de compilação
swiftmagex icon bg.png mark.png,scale=0.8,dy=-20 \
  --fill auto:#7B1FA2 -o AppIcon.icon --flat-preview --validate

# Selo fixado no canto inferior direito, sem vidro, tinta branca, grupo próprio
swiftmagex icon art.png badge.png,glass=false,fill=#FFFFFF,dx=222,dy=223,group=2
```

Arraste o `AppIcon.icon` resultante para um projeto do Xcode 26 (ou
abra no Icon Composer) — o Xcode renderiza o efeito Liquid Glass e gera
as variantes planas para versões mais antigas do sistema.
...
`swiftmagex-mcp` expõe dez ferramentas — `generate_image`, `edit_image`,
`resize_image`, `overlay_text`, `composite_images`, `appstore_screenshots`,
`list_frames`, `remove_background`, `smart_crop` e `compose_icon` — por
stdio. Os argumentos espelham as
...
`appstore_screenshots`. `compose_icon` espelha `swiftmagex icon`
(camadas como array de objetos) e devolve a prévia plana como conteúdo
de imagem quando `flat_preview` é passado — o fluxo natural do agente é
`generate_image` → `remove_background` → `compose_icon`.
...
Composer). Qualquer coisa fora desse
...
MIT. Consulte [LICENSE](LICENSE) para o texto completo.
