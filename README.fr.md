...
Huit sous-commandes, toutes partageant les mêmes flags globaux :
...
| `--version` | Imprime `0.2.0` et quitte. |
...
**Choisissez le modèle selon vos exigences de qualité.** Les modèles sont
interchangeables à chaque appel via `--model` — rien d'autre ne change dans
la commande. Le modèle par défaut `gemini-2.5-flash-image` est le
généraliste stable ; le tout dernier `gemini-3.1-flash-image-preview`
produit des résultats nettement plus détaillés et choisit de lui-même des
cadrages plus larges, non carrés ; la famille Imagen offre un contrôle
explicite du ratio d'aspect (`--size` est traduit en `aspectRatio`), avec
`imagen-4.0-fast-generate-001` optimisé pour la vitesse et
`imagen-4.0-ultra-generate-001` pour une fidélité maximale (une image par
appel). Par exemple, `gemini-3.1-flash-image-preview` a produit cette image
1408×768 :
...
<img src="docs/images/example-generate-fox-gemini31.png" alt="Renard détaillé dans une forêt d'automne, généré par gemini-3.1-flash-image-preview" width="480" />
...
| `--seed <uint64>` | — | Enregistré dans les métadonnées même si le fournisseur l'ignore. |
| `--model <id>` | `gemini-2.5-flash-image` | Doit être un modèle Gemini — la forme `:predict` d'Imagen n'accepte pas d'entrées image inline et est rejetée avec le code de sortie 2. |
...
Composition multi-images — assembler un sujet avec une scène via `--reference` :

| Entrée (sujet) | Référence (scène) | Résultat |
|---|---|---|
| <img src="examples/canoe.png" alt="Canoë en bois sur fond blanc" width="200"> | <img src="examples/mountain.png" alt="Lac de montagne à l'aube" width="200"> | <img src="examples/canoe-on-mountain-lake.png" alt="Canoë flottant sur le lac de montagne" width="200"> |
...
Détoure le sujet saillant au premier plan et le laisse sur un arrière-plan
transparent, à l'aide de la segmentation on-device de Vision — sans API
d'IA, sans clé, sans consommation de quota. Le résultat comporte toujours
un canal alpha, il est donc écrit en PNG (une extension `--output`
différente de `.png` est convertie en `.png`).
...
| `-o`, `--output <chemin>` | voisin de la source | Toujours écrit en PNG. |

```sh
# Détourer le sujet sur un arrière-plan transparent
swiftmagex remove-bg photo.jpg -o cutout.png

# Par défaut, un PNG voisin de la source
swiftmagex remove-bg product.heic
```

Si aucun sujet saillant au premier plan n'est détecté, la commande échoue
avec une erreur raster (exit 1).

### `swiftmagex crop` — recadrage par ratio sensible à la saillance

Recadre vers un ratio d'aspect fourni par l'utilisateur en centrant la
fenêtre de recadrage sur le sujet saillant détecté par le modèle
d'attention on-device de Vision — et non sur le centre géométrique. Sans
API IA, sans clé, sans consommation de quota. La sortie conserve l'échelle
de pixels de la source (c'est un recadrage, pas un redimensionnement) et
utilise par défaut le même format.
...
| Option | Par défaut | Notes |
|---|---|---|
| `<input>` | — | PNG, JPEG, HEIC ou WebP. Écriture PNG / JPEG uniquement. |
| `--aspect <W:H>` | — | Obligatoire. Deux entiers positifs, p. ex. `1:1`, `4:5`, `9:16`. |
| `-o`, `--output <chemin>` | voisin de la source | |
| `--format <png\|jpeg>` | identique à la source | HEIC / WebP retombent sur PNG par défaut. |
| `--quality <0.0–1.0>` | `0.9` | JPEG uniquement. |

```sh
# Recadrage carré centré sur le sujet saillant
swiftmagex crop photo.jpg --aspect 1:1

# Recadrage portrait 9:16, ré-encodé en JPEG
swiftmagex crop photo.jpg --aspect 9:16 -o portrait.jpg --format jpeg --quality 0.85
```

Lorsque la saillance ne détecte aucun objet saillant (rare ; images plates
ou uniformes), retombe sur un recadrage centré afin que le ratio demandé
soit tout de même respecté.

### `swiftmagex icon` — paquets `.icon` d'Icon Composer

Assemble un paquet `.icon`
d'[Icon Composer](https://developer.apple.com/icon-composer/) — le format
d'icônes d'app en couches Liquid Glass pour iOS 26+ / macOS 26+ — à
partir d'images de couches préparées. Entièrement local, aucune clé. Les
couches sont listées de bas en haut et empilées sur le canevas de
1024 pt d'Icon Composer ; la commande écrit `icon.json` plus `Assets/`
et renvoie le chemin absolu du paquet. Préparez d'abord les couches avec
`generate`, `remove-bg` ou `resize`.

```
swiftmagex icon <layer[,key=value...]>... [options]
```

Les options par couche s'ajoutent au chemin, séparées par des virgules :
`name=…`, `glass=true|false` (Liquid Glass, true par défaut), `scale=N`
(multiplicateur de la taille naturelle de la couche, 1 pixel source =
1 point), `dx=N` / `dy=N` (décalage en points depuis le placement
centré, positif = droite/bas), `fill=#RRGGBB[AA]` (teinte unie),
`group=N` (à partir de 1 ; les couches d'un groupe doivent être
contiguës, max. 4 par groupe).

| Option | Défaut | Notes |
|---|---|---|
| `<layer>...` | — | De bas en haut. PNG avec transparence recommandé ; les autres formats sont ré-encodés en PNG. |
| `-o`, `--output <path>` | `AppIcon.icon` | `.icon` est ajouté s'il manque. |
| `--fill <solid:#HEX\|auto:#HEX>` | `solid:#FFFFFF` | Arrière-plan de l'icône ; `auto:` est le dégradé système issu d'une couleur. |
| `--overwrite` | off | Remplace atomiquement un paquet existant. |
| `--flat-preview` | off | Écrit en plus un composite plat PNG 1024×1024 (`<name>-flat.png`) — sans Liquid Glass, sans masque squircle — pour les READMEs et les consommateurs sans Xcode. |
| `--flat-preview-output <path>` | à côté du paquet | |
| `--validate` | off | Vérifie le paquet en le compilant avec `actool` de Xcode (nécessite Xcode 26). Code 4 si actool est introuvable, code 2 si le paquet ne compile pas. |

```sh
# Fond + marque en verre, remplissage dégradé, avec aperçu et vérification de compilation
swiftmagex icon bg.png mark.png,scale=0.8,dy=-20 \
  --fill auto:#7B1FA2 -o AppIcon.icon --flat-preview --validate

# Badge ancré en bas à droite, sans verre, teinte blanche, groupe dédié
swiftmagex icon art.png badge.png,glass=false,fill=#FFFFFF,dx=222,dy=223,group=2
```

Déposez le `AppIcon.icon` obtenu dans un projet Xcode 26 (ou ouvrez-le
dans Icon Composer) — Xcode rend l'effet Liquid Glass et génère les
variantes plates pour les versions d'OS antérieures.
...
Erreur (`resize`, `text`, `remove-bg` et `crop` omettent `provider` / `model`) :
...
`swiftmagex-mcp` expose dix outils — `generate_image`, `edit_image`,
`resize_image`, `overlay_text`, `composite_images`, `appstore_screenshots`,
`list_frames`, `remove_background`, `smart_crop` et `compose_icon` — via
stdio. Les arguments des outils
...
répertoire de travail du serveur. `generate_image` et `edit_image`
retournent en plus les octets de l'image en contenu MCP `image` afin que le
modèle puisse inspecter ce qui a été produit. `list_frames` énumère les
cadres d'appareil embarqués dont les identifiants sont acceptés par
l'argument `frame` de `appstore_screenshots`. `compose_icon` reprend
`swiftmagex icon` (couches sous forme de tableau d'objets) et renvoie
l'aperçu plat en contenu image quand `flat_preview` est activé — le flux
naturel pour un agent est `generate_image` → `remove_background` →
`compose_icon`.
...
| `I/O error: input file not found: /…/foo.png` (exit 1) | Le chemin passé à une commande locale (`resize` / `text` / `composite` / `appstore` / `remove-bg` / `crop`, ou leurs outils MCP) n'existe pas ou est illisible. | Donnez un chemin absolu, vérifiez les permissions, et confirmez que le format est PNG / JPEG / HEIC / WebP (écriture en PNG / JPEG seulement). |
...
paquets `.icon` d'Icon Composer). Tout ce qui sort
de ce périmètre est différé ; la liste
